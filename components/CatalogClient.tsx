...
export default function CatalogClient({ properties }: { properties: Property[] }) {
  ...
  const filtered = properties.filter(p => {
    const locMatch = location === 'All' || p.location?.toLowerCase().includes(location.toLowerCase()) || p.neighborhood?.toLowerCase().includes(location.toLowerCase())
  ...
  return (
    <main style={{ background: '#F5F2EE', minHeight: '100vh', padding: '48px 24px' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        ...
        <p style={{ color: '#6B6B6B', fontFamily: 'Poppins', fontSize: 14, marginBottom: 40 }}>
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 40 }}>
          {[
            ...
            <select key={f.label} value={f.value} onChange={e => f.set(e.target.value)}
              style={{ padding: '10px 16px', border: '1px solid #E8E3DC', borderRadius: 8, background: '#fff', fontFamily: 'Poppins', fontSize: 13, color: '#1A1A1A', cursor: 'pointer' }}>
              {f.options.map(o => <option key={o} value={o}>{o === 'All' ? `${f.label}: All` : o.charAt(0).toUpperCase() + o.slice(1)}</option>)}
            </select>
          ))}
          <span style={{ padding: '10px 0', color: '#6B6B6B', fontSize: 13, fontFamily: 'Poppins' }}>
        ...
        {filtered.length === 0 && (
          <div style={{ textAlign: 'center', padding: '80px 0', color: '#6B6B6B', fontFamily: 'Poppins' }}>
